...
public class CMDSignatureController {
	...
	public String requestSignature(Model model, HttpServletRequest response,
		...
		signatureDocumentForm.setCertificate(certificates.get(0));
		signatureDocumentForm.setCertificateChain(certificates.subList(1, certificates.size()));
		...
		SignatureAlgorithm certificateSignatureAlgorithm = DSSUtils.loadCertificate(signatureDocumentForm.getCertificate()).getSignatureAlgorithm();
	...
	public String signDocument(Model model, HttpServletRequest response,
		...
		byte[] signature = cmdService.validateOtp(signatureDocumentForm.getProcessId(), cmdOtpForm.getUserOtp());
